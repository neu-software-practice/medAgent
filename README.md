...
- **唯一公开包** `medagent`（Go module `medagent`），以 HTTP/JSON 对外；底层决策层在 `internal/`，外部不可见、不可直接调用。
- **Go 1.22，零外部依赖**（仅标准库）。
...
```
medagent/                  # 唯一公开包：facade（Service + HTTP Handler）
...
依赖方向：`medagent → internal/{ai,openaicompat,consultlog}`，`internal` 外部不可见。
...
```go
svc, _ := medagent.New(medagent.Config{Provider: "deepseek", APIKey: key, Model: "deepseek-chat", LogDir: "./logs"})
defer svc.Close()
