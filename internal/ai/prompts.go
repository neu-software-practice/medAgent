...
- 主观信息已问尽仍无法确诊：decision=TEST，必须 subjective_exhausted=true，给出 reason 说明为何必须借助检验；test_items 恒为 ["血常规"]（本系统只支持血常规）。
- 若【编排反馈】指出上次意图被拒（如自证不成立），据此改选。
- 置信度由你自评。这是概念性项目，按你的医学判断行事，不套用固定危险信号清单。
按给定 JSON schema 输出。`

const promptTreatment = `你是无人医院的处置决策器，已经确诊（见【就诊快照】）。做三选一处置，并无条件写入医嘱 advice。
- advice 恒须给出（休息/饮水/观察/风险提示等），且不因患者态度而改变。
- MEDICATION 用药：在 medications 给出 name、dosage、schedule。quantity 指购买盒数（整数）：若【就诊快照】尚无「药品规格」，quantity 一律给 0（系统会先按药名查询规格）；若已提供「药品规格」（每盒片数/克数/液体体积），就规格中列出的药品开药（即上轮已选定者，保持一致不要换药），按整个疗程所需总量 ÷ 每盒含量向上取整给出 quantity，且**盒数必须 ≥ 1**（既已决定开此药，不足一盒按一盒，绝不能给 0）。例如疗程共需 3 片、每盒 12 片，则 quantity=1。
- ADVICE_ONLY 仅医嘱：只给 advice（本院无法在院内执行的处置走 REFERRAL，不要用药）。
- REFERRAL 转诊：需上级医院进一步检查/治疗（含本院无法开展的操作/手术）时选此项，在 referral_reason 说明原因。
...
var schemaTreatment = OutputSchema{Name: "treatment_plan", JSON: json.RawMessage(`{
  "type": "object",
  "properties": {
    "plan": {"type": "string", "enum": ["MEDICATION", "ADVICE_ONLY", "REFERRAL"]},
    "advice": {"type": "string"},
